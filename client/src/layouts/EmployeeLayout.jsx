...
const TABS = [
  ...
  { key: '/my-bookings', labelKey: 'nav.myBookings', icon: '📋' },
  { key: '/notifications', labelKey: 'nav.alerts', icon: '🔔' },
  ...
  return (
    <div style={{ maxWidth: 480, margin: '0 auto', minHeight: '100vh', display: 'flex', flexDirection: 'column', background: '#f5f7fb' }}>
      <Layout.Header style={{ display: 'flex', alignItems: 'center', gap: 8, paddingInline: 12 }}>
        <Typography.Text style={{ color: '#fff', fontWeight: 600, flex: 1 }}>{t('app.carBooking')}</Typography.Text>
        <LanguageSelector />
        <NotificationBell />
        <FeedbackButton />
